...
class TransactionLogic {
    ...
    public:
        ...
        bool NapTien(std::string soTK, double soTien);
        
        //RÚT TIỀN
        // Cần kiểm tra mã PIN trước khi rút
        bool RutTien(std::string soTK, std::string maPIN, double soTien);

        //CHUYỂN KHOẢN
        // Kiểm tra mã PIN của TK gửi, kiểm tra số dư đủ, kiểm tra Tk nhân có tồn tại
        bool ChuyenKhoan(std::string soTKGui, std::string maPIN, std::string soTKNhan, double soTien);
